...
`NOTE: MAP, FILTER, REDUCE  allows the chaining`

**8. array.filter()**:

The filter() method creates a new array with all elements that pass the test implemented by the provided function. It takes each element in the array and it apply the condition statement against them. If the conditional statements return true, the element get pushed to the output array
and if the condition return false then it will not push to the output array. So basically it return the element which fullfill the condition.
...
nums.reduce((accumulator, currentValue, index, array) => {}, initial value)
0 -> Initial value
accumulator -> Accumulator is the computaion of the previous value

NOTE: If there is no initial value, It takes first element of array as value for accumulator
...
In a programing lang where function is treated as a variable this is known as **first-class function**
...
A closure is a function that refrences the variable to it's outerscope from it's inner scope. However the reverse is not true ie the variable declared in the function is not accessible outside it's scope.

A closure is the combination of a function and the lexical environment within which that function was declared. i.e, it is an inner function that has access to the outer or enclosing function\'s variables. In js closures are created, at function creation time.
